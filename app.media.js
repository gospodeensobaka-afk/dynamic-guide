...
function showFullscreenMedia(src, type, duration = null) {
    ...
    if (type === "video") {
        // FIX задержка + iOS затемнение:
        // Берём прогретый warmup элемент если есть — декодер уже разогрет, старт мгновенный
        ...
        // Всё это ДО replaceChild — иначе iOS затемняет экран
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    notReadyBtn.onclick = () => {
        ...
        // Пересобираем плоскую ленту фото из последних 3 зон
        galleryFlatPhotos = [];
